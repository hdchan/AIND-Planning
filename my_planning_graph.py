...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if node_a1.action.effect_rem == node_a2.action.precond_pos and node_a1.action.effect_rem:
            return True
        if node_a1.action.effect_add == node_a2.action.precond_pos and node_a1.action.effect_add:
            return True
        if node_a2.action.effect_rem == node_a1.action.precond_pos and node_a2.action.effect_rem:
            return True
        if node_a2.action.effect_add == node_a1.action.precond_pos and node_a2.action.effect_add:
            return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent_2 in node_a2.parents:
            for parent_1 in list(node_a1.parents):
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent_2 in node_s2.parents:
            for parent_1 in list(node_s1.parents):
                if parent_1.is_mutex(parent_2):
                    is_mutex = True
    ...
    def h_levelsum(self) -> int:
        ...
        for level in range(len(self.s_levels)):
            for state in self.s_levels[level]:
                if state.symbol in goal_copy and state.is_pos:
                    level_sum += level
                    goal_copy.remove(state.symbol)
                    if not goal_copy:
                        return level_sum
